...
function ListCard(el){
	...
	var points=-1,
		...
		$blockBadge=$('<div class="badge badge-points point-count" style="background-image: url('+blockIconUrl+'); background-color: #AF0000 !important;border-color: #870000 !important;"/>')
			.bind('DOMSubtreeModified DOMNodeRemovedFromDocument',function(e){
				if(busy4)return;
				busy4=true;
				clearTimeout(to4);
	...
	function getBlock()
	{
		...
		if(!$title[0]||busy)return;
		busy=true;
		var title=$title[0].text, toLower = title.toLowerCase(), blockExists = (toLower.indexOf('(block) ') === 0);
		if(blockExists)
		{
			if($card.parent()[0]){
				$title[0].textContent = title.substring(8,title.length);
	...
	this.__defineGetter__('block',function(){
		//don't add to total when filtered out
		return parsed&&(!filtered||($card.css('opacity')==1 && $card.css('display')!='none'))?block:''
	});
